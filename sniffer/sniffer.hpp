...
namespace {
    ...
    public:
        static void Init(Napi::Env env) {
            ...
            Napi::Function ctor = DefineClass(env, "Sniffer", {
                    InstanceMethod("setFilter", &Sniffer::setFilter),
                    InstanceMethod("start", &Sniffer::start),
//                    InstanceMethod("stop", &Sniffer::stop),
                    InstanceMethod("restart", &Sniffer::restart),
                    InstanceMethod("onProgress", &Sniffer::onProgress),
                    InstanceMethod("setFilter", &Sniffer::setFilter),
                    InstanceMethod("keepAlive", &Sniffer::keepAlive),
                    InstanceMethod("closeDev", &Sniffer::closeDev)
            });
        ...
        static Napi::Object NewInstance(const Napi::CallbackInfo &info) {
            ...
            if (length <= 0 || !info[0].IsString()) {
                Napi::TypeError::New(env, "The first parameter must be a device name as a string").ThrowAsJavaScriptException();
            }
            ...
            if (length >= 2) {
                if (!info[1].IsObject()) {
                    Napi::TypeError::New(env, "The second parameter must be pcap attributes as a object").ThrowAsJavaScriptException();
                } else {
        ...
        explicit Sniffer(const Napi::CallbackInfo &info) : Napi::ObjectWrap<Sniffer>(info) {
            Napi::Env env = info.Env();
            Napi::HandleScope scope(env);
            string devName = info[0].ToString().Utf8Value();
            ...
            this->pcapHandle = pcap_create(devName.data(), this->errBuf);
